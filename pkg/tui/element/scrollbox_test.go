...
func TestElement_ScrollModes(t *testing.T) {
	...
	for name, tt := range tests {
		t.Run(name, func(t *testing.T) {
			...
			e.ScrollTo(0, 0)
			downEvent := tui.KeyEvent{Key: tui.KeyDown}
			...
			e.ScrollTo(0, 0)
			rightEvent := tui.KeyEvent{Key: tui.KeyRight}
...
func TestElement_ScrollUnhandledEvent(t *testing.T) {
	...
	event := tui.KeyEvent{Key: tui.KeyRune, Rune: 'a'}
	handled := e.HandleEvent(event)
...
func TestElement_UserHandlerCalledFirst(t *testing.T) {
	...
	e := New(
		WithHeight(10),
		WithScrollable(ScrollVertical),
		WithOnEvent(func(event tui.Event) bool {
			handlerCalled = true
	...
	// Send a scroll event
	event := tui.KeyEvent{Key: tui.KeyDown}
	e.HandleEvent(event)
